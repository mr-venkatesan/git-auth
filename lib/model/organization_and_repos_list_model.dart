class OrganizationAndReposListModel {
  List<Org> orgs;
  List<Repo> repos;
  Org selectedOrg;

  OrganizationAndReposListModel({
    required this.orgs,
    required this.repos,
    required this.selectedOrg,
  });

  factory OrganizationAndReposListModel.fromJson(Map<String, dynamic> json) => OrganizationAndReposListModel(
    orgs: List<Org>.from(json["organizations"].map((x) => Org.fromJson(x))),
    repos: List<Repo>.from(json["repos"].map((x) => Repo.fromJson(x))),
    selectedOrg: Org.fromJson(json["selectedOrganizations"]),
  );

  Map<String, dynamic> toJson() => {
    "organizations": List<dynamic>.from(orgs.map((x) => x.toJson())),
    "repos": List<dynamic>.from(repos.map((x) => x.toJson())),
    "selectedOrganizations": selectedOrg.toJson(),
  };
}
...
class SecurityAndAnalysis {
  DependabotSecurityUpdates secretScanning;
  DependabotSecurityUpdates secretScanningPushProtection;
  DependabotSecurityUpdates dependabotSecurityUpdates;
  DependabotSecurityUpdates secretScanningNonProviderPatterns;
  DependabotSecurityUpdates secretScanningValidityChecks;

  SecurityAndAnalysis({
    required this.secretScanning,
    required this.secretScanningPushProtection,
    required this.dependabotSecurityUpdates,
    required this.secretScanningNonProviderPatterns,
    required this.secretScanningValidityChecks,
  });

  factory SecurityAndAnalysis.fromJson(Map<String, dynamic> json) => SecurityAndAnalysis(
    secretScanning: DependabotSecurityUpdates.fromJson(json["secret_scanning"]),
    secretScanningPushProtection: DependabotSecurityUpdates.fromJson(json["secret_scanning_push_protection"]),
    dependabotSecurityUpdates: DependabotSecurityUpdates.fromJson(json["dependabot_security_updates"]),
    secretScanningNonProviderPatterns: DependabotSecurityUpdates.fromJson(json["secret_scanning_non_provider_patterns"]),
    secretScanningValidityChecks: DependabotSecurityUpdates.fromJson(json["secret_scanning_validity_checks"]),
  );
...
class DependabotSecurityUpdates {
  ...
  factory DependabotSecurityUpdates.fromJson(Map<String, dynamic> json) => DependabotSecurityUpdates(
